...
## Self-healing schema
The Worker runs a non-destructive schema reconciler before authenticated API operations. It creates only the known HR Core operational tables and indexes when missing and seeds required permission definitions. It does not drop arbitrary tables or business data. The admin maintenance action removes only known orphan records.
